...
function PickupStatusBadge({ invoice, pickupTrackingAvailable }) {
  if (!pickupTrackingAvailable) {
    return (
      <span className="inline-flex rounded-full px-2.5 py-1 text-[11px] font-semibold ring-1 bg-amber-50 text-amber-700 ring-amber-100">
        Not tracked
      </span>
    )
  }
  ...
  return (
    <span className={`inline-flex rounded-full px-2.5 py-1 text-[11px] font-semibold ring-1 ${pickedUp ? 'bg-gym-50 text-gym-700 ring-gym-100' : 'bg-slate-100 text-slate-600 ring-slate-200'}`}>
