  ...
  return (
    <button
      ...
      className={`w-full rounded-3xl border p-4 text-left transition ${
        active ? 'border-gym-400 bg-gym-50' : 'border-slate-200 bg-white hover:border-gym-300 hover:bg-slate-50'
      }`}
    >
      ...
      <div className="mt-3 grid gap-2 text-xs text-slate-600 sm:grid-cols-2">
        <div>
          <span className="font-semibold text-slate-900">Membership:</span> {item.planName || item.membershipStatus || 'None'}
        </div>
        <div>
          <span className="font-semibold text-slate-900">Next PT:</span> {item.nextSessionDate || 'None'}
        </div>
        <div>
          <span className="font-semibold text-slate-900">Replacement:</span> {item.replacementStatus || 'None'}
        </div>
        <div>
          <span className="font-semibold text-slate-900">Last notification:</span> {item.lastNotificationAt ? formatDate(item.lastNotificationAt) : 'None'}
        </div>
      </div>
    </button>
  )
      ...
      <header className="overflow-hidden rounded-[32px] border border-white/10 bg-[radial-gradient(circle_at_top_right,_rgba(245,158,11,0.16),_transparent_32%),linear-gradient(135deg,_rgba(18,18,26,0.98),_rgba(10,10,15,0.92))] p-6 shadow-[0_24px_60px_rgba(0,0,0,0.32)] backdrop-blur-xl">
          ...
          <div>
            <h2 className="gc-section-kicker">Customer lifecycle view</h2>
              <h3 className="mt-3 text-3xl font-bold tracking-tight text-slate-900">Read the whole customer state before taking action</h3>
              <p className="mt-3 max-w-3xl text-sm leading-7 text-slate-600">Read-first support console for triage and fast deep-links into existing admin and reception screens.</p>
          </div>
            <div className="space-y-3">
              <div className="rounded-full border border-gym-200 bg-gym-50 px-4 py-2 text-xs font-semibold uppercase tracking-[0.18em] text-gym-800">
                Support read-only
              </div>
              <div className="rounded-3xl border border-white/70 bg-white/85 p-4">
                ...
                <p className="mt-2 text-sm font-semibold text-slate-900">Diagnose quickly, then deep-link to the right workspace</p>
              </div>
            </div>
          </div>
        </header>
        ...
        <div className="grid gap-5 xl:grid-cols-[380px_minmax(0,1fr)]">
          <aside className="space-y-3">
            <div className="flex items-center justify-between">
              <p className="text-sm font-semibold text-slate-700">Matched customers</p>
              <span className="text-xs text-slate-500">{searchItems.length}</span>
            </div>
            {searchQuery.isLoading ? <p className="text-sm text-slate-500">Searching customers...</p> : null}
            {searchQuery.isError ? <p className="text-sm text-rose-600">Could not search customers.</p> : null}
            <div className="space-y-3">
          ...
          <div className="space-y-5">
            {detailQuery.isLoading ? <p className="text-sm text-slate-500">Loading support detail...</p> : null}
            {detailQuery.isError ? <p className="text-sm text-rose-600">Could not load support detail.</p> : null}
              ...
              <>
                <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
                  <InfoCard icon={UserRound} title={account.fullName || 'Customer'} subtitle={account.email || account.phone || 'No contact on file'}>
                    <p className="text-sm text-slate-600">Account state: <span className="font-semibold text-slate-900">{account.locked ? 'Locked' : account.active === false ? 'Inactive' : 'Active'}</span></p>
                    <p className="mt-1 text-xs text-slate-500">Created: {formatDate(account.createdAt)}</p>
                  </InfoCard>

                  <InfoCard icon={Clock3} title="Current membership" subtitle={memberships.current?.planName || 'No membership'}>
                    <p className="text-sm text-slate-600">Status: <span className="font-semibold text-slate-900">{memberships.current?.status || 'NONE'}</span></p>
                    <p className="mt-1 text-xs text-slate-500">Ends: {memberships.current?.endDate || '-'}</p>
                  </InfoCard>

                  <InfoCard icon={LifeBuoy} title="PT context" subtitle={pt.currentPhase?.coachName || 'No active PT phase'}>
                    <p className="text-sm text-slate-600">Phase: <span className="font-semibold text-slate-900">{pt.currentPhase?.status || 'NONE'}</span></p>
                    <p className="mt-1 text-xs text-slate-500">Upcoming sessions: {(pt.upcomingSessions || []).length}</p>
                  </InfoCard>

                  <InfoCard icon={PackageSearch} title="Commerce & pickup" subtitle={`${pickup.count || 0} awaiting pickup`}>
                    <p className="text-sm text-slate-600">Recent orders: <span className="font-semibold text-slate-900">{orders.count || 0}</span></p>
                    <p className="mt-1 text-xs text-slate-500">Invoice email failures: {invoiceEmail.failureCount || 0}</p>
                  </InfoCard>
                </section>
                ...
                {alerts.length ? (
                  <section className="rounded-3xl border border-amber-200 bg-amber-50 p-5">
                    <div className="flex items-center gap-2">
                      ...
                      <h3 className="text-base font-bold text-slate-900">Support alerts</h3>
                    </div>
                    <div className="mt-4 grid gap-3 md:grid-cols-2">
                      {alerts.map((alert) => (
                        <div key={alert.id} className="rounded-2xl border border-amber-200 bg-white px-4 py-3">
                          <p className="text-sm font-semibold text-slate-900">{alert.message}</p>
                          <a href={alert.route} className="mt-2 inline-block text-xs font-semibold text-amber-800">
                  ...
                  <InfoCard icon={ShieldAlert} title="Account & memberships" subtitle="Account state, lock reason, and latest membership rows">
                    <div className="space-y-3">
                      <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4 text-sm text-slate-600">
                        <p>Email: <span className="font-semibold text-slate-900">{account.email || '-'}</span></p>
                        <p className="mt-1">Phone: <span className="font-semibold text-slate-900">{account.phone || '-'}</span></p>
                        <p className="mt-1">Lock reason: <span className="font-semibold text-slate-900">{account.lockReason || 'None'}</span></p>
                      </div>
                      {(memberships.history || []).map((item) => (
                        <div key={item.customerMembershipId} className="rounded-2xl border border-slate-200 bg-white p-4 text-sm text-slate-600">
                  ...
                  <InfoCard icon={LifeBuoy} title="PT lifecycle" subtitle="Requests, upcoming sessions, and replacement context">
                    <div className="space-y-3">
                      {(pt.requests || []).map((item) => (
                        <div key={`request-${item.ptRequestId}`} className="rounded-2xl border border-slate-200 bg-slate-50 p-4 text-sm text-slate-600">
                          <p className="font-semibold text-slate-900">Request #{item.ptRequestId}</p>
                          <p className="mt-1">Status: {item.status}</p>
                      ...
                      {(pt.upcomingSessions || []).map((item) => (
                        <div key={`session-${item.ptSessionId}`} className="rounded-2xl border border-slate-200 bg-white p-4 text-sm text-slate-600">
                          <p className="font-semibold text-slate-900">{item.sessionDate || '-'} with {item.coachName || 'Coach'}</p>
                  ...
                  <InfoCard icon={PackageSearch} title="Orders & pickup" subtitle="Recent orders and awaiting pickup">
                    <div className="space-y-3">
                      {(orders.items || []).map((item) => (
                        <div key={`order-${item.orderId}`} className="rounded-2xl border border-slate-200 bg-slate-50 p-4 text-sm text-slate-600">
                          <p className="font-semibold text-slate-900">Order #{item.orderId}</p>
                          <p className="mt-1">Status: {item.status || 'Unknown'}</p>
                          <p className="mt-1">Invoice: {item.invoiceCode || 'Not issued'}</p>
                          <p className="mt-1">Paid: {formatDate(item.paidAt)}</p>
                        </div>
                      ))}
                  ...
                  <InfoCard icon={MailWarning} title="Invoice email" subtitle="Delivery tracking for product receipts">
                    <div className="space-y-3">
                      {(invoiceEmail.items || []).map((item) => (
                        <div key={`invoice-${item.invoiceId}`} className="rounded-2xl border border-slate-200 bg-slate-50 p-4 text-sm text-slate-600">
                          <p className="font-semibold text-slate-900">{item.invoiceCode || `Invoice #${item.invoiceId}`}</p>
                          <p className="mt-1">Recipient: {item.recipientEmail || 'N/A'}</p>
                          <p className="mt-1">Sent: {formatDate(item.emailSentAt)}</p>
                  ...
                  <InfoCard icon={Bell} title="Notifications" subtitle="Latest customer-facing notifications">
                    <div className="space-y-3">
                      {(notifications.items || []).map((item) => (
                        <div key={`notification-${item.notificationId}`} className="rounded-2xl border border-slate-200 bg-slate-50 p-4 text-sm text-slate-600">
                          <p className="font-semibold text-slate-900">{item.title}</p>
                          <p className="mt-1 line-clamp-3">{item.message}</p>
                          <p className="mt-1 text-xs text-slate-500">{formatDate(item.createdAt)} | {item.read ? 'Read' : 'Unread'}</p>
                        </div>
                      ))}
