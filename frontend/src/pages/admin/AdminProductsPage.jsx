        ...
        <section className="gc-card-compact space-y-4">
          <header className="flex flex-wrap items-center justify-between gap-3 border-b border-white/10 pb-4">
            <div>
              ...
              <p className="mt-1 text-xs text-zinc-500">Filter by category, status, and review activity before editing.</p>
            </div>
            <button
              type="button"
              onClick={handleCreate}
              className="gc-button-primary px-4 py-2 text-xs"
            >
              <PlusCircle size={14} />
              New product
            </button>
          ...
          <div className="grid gap-3 lg:grid-cols-[minmax(0,1.6fr)_repeat(3,minmax(0,1fr))]">
            <label className="flex items-center gap-2 rounded-2xl border border-white/10 bg-white/5 px-3 py-2 transition-[border-color,background-color,box-shadow] focus-within:border-gym-500/40 focus-within:bg-white/[0.07] focus-within:shadow-[0_0_0_2px_rgba(245,158,11,0.14)]">
              ...
              <input
                ...
                className="w-full bg-transparent text-sm text-white placeholder:text-slate-400 focus-visible:outline-none"
              />
            </label>
            <select
              name="admin-product-status-filter"
              value={statusFilter}
            ...
            <select
              name="admin-product-review-filter"
              value={reviewFilter}
            ...
            <select
              name="admin-product-category-filter"
          ...
          <div className="space-y-3">
            ...
            {!productsQuery.isLoading && filteredProducts.length === 0 ? (
              <p className="text-sm text-zinc-500">No products match the current filters.</p>
            ) : null}
            {filteredProducts.map((product) => (
              <article key={product.productId} className="rounded-3xl border border-white/10 bg-[rgba(18,18,26,0.92)] p-4 shadow-sm">
                <div className="flex gap-4">
                  <div className="h-24 w-24 overflow-hidden rounded-3xl bg-white/10">
                    {product.thumbnailUrl ? (
                      <img src={product.thumbnailUrl} alt={product.name} width="96" height="96" loading="lazy" className="h-full w-full object-cover" />
                    ) : null}
                  </div>
                  <div className="min-w-0 flex-1">
                    <div className="flex flex-wrap items-start justify-between gap-3">
                      <div>
                        ...
                        <p className="mt-1 text-sm text-slate-400">{product.shortDescription || product.description}</p>
                      </div>
                      <span className={`rounded-full px-3 py-1 text-[11px] font-semibold ${
                        product.active ? 'bg-emerald-500/10 text-emerald-300' : 'bg-white/10 text-slate-400'
                      }`}>
                    ...
                    <div className="mt-3 flex flex-wrap gap-2">
                      {(product.categories || []).map((category) => (
                        <span key={category.productCategoryId} className="rounded-full bg-white/10 px-3 py-1 text-[11px] font-semibold text-slate-400">
                          {category.name}
                        </span>
                      ))}
                    </div>
                    ...
                    <div className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
                      <ProductStat label="Price" value={formatCurrency(product.price || 0)} />
                      <ProductStat label="Categories" value={formatInteger((product.categories || []).length)} />
                      <ProductStat label="Reviews" value={formatInteger(product.reviewCount ?? 0)} />
                    ...
                    <div className="mt-4 flex flex-wrap gap-2">
                      <button
                        ...
                        className="inline-flex items-center gap-2 rounded-full border border-white/10 bg-[rgba(18,18,26,0.92)] px-3 py-2 text-xs font-semibold text-slate-300 hover:bg-white/5"
                      >
                        <Edit3 size={13} />
                        Edit
                      </button>
                      {product.active ? (
                        <button
                          ...
                          className="inline-flex items-center gap-2 rounded-full border border-rose-500/20 bg-rose-500/10 px-3 py-2 text-xs font-semibold text-rose-300 hover:bg-rose-100"
                        >
                          <PackageX size={13} />
                          Archive
                        </button>
                        ...
                        <button
                          ...
                          className="inline-flex items-center gap-2 rounded-full border border-gym-500/20 bg-gym-500/10 px-3 py-2 text-xs font-semibold text-gym-300 hover:bg-gym-500/15"
                        >
        ...
        <div className="space-y-6">
          <section className="gc-card-compact space-y-5">
            <header className="border-b border-white/10 pb-4">
              ...
              <p className="mt-1 text-xs text-zinc-500">Keep the form structured: basic info, pricing, categories, customer-facing copy, then gallery.</p>
            </header>
            ...
            {editingProduct ? (
              <form noValidate onSubmit={handleSubmit} className="space-y-5">
                          ...
                          <button
                            ...
                            className={`rounded-full px-3 py-2 text-xs font-semibold ${
                              active
                                ...
                                : 'border border-white/10 bg-[rgba(18,18,26,0.92)] text-slate-400 hover:bg-white/5'
                            }`}
                          >
                            {category.name}
                          </button>
                  ...
                  <Field label={`Gallery images (${editingProduct.images.length}/8)`}>
                    <div className="space-y-3">
                      {editingProduct.images.map((image, index) => (
                        <div key={`${index}-${image.displayOrder}`} className="rounded-2xl border border-white/10 bg-white/5 p-3">
                          <div className="grid gap-3 md:grid-cols-[96px_minmax(0,1fr)]">
                            ...
                            <div className="space-y-3">
                              <div className="flex flex-wrap items-center gap-2">
                                <label className="inline-flex cursor-pointer items-center gap-2 rounded-full border border-white/10 bg-[rgba(18,18,26,0.92)] px-3 py-2 text-xs font-semibold text-slate-300 hover:bg-white/5">
                                  <Upload size={14} />
                                ...
                                {image.imageUrl ? <span className="text-[11px] text-zinc-500">Uploaded</span> : <span className="text-[11px] text-zinc-500">No image uploaded yet</span>}
                              </div>
                              <input
                                type="text"
                                name={`altText-${index + 1}`}
                                autoComplete="off"
                                value={image.altText}
                          ...
                          <div className="mt-3 flex flex-wrap items-center justify-between gap-2">
                            <div className="flex flex-wrap items-center gap-2">
                              <button
                                ...
                                className={`rounded-full px-3 py-1 text-xs font-semibold ${
                                  image.isPrimary ? 'bg-gym-600 text-white' : 'border border-white/10 bg-[rgba(18,18,26,0.92)] text-slate-400'
                                }`}
                              >
                              ...
                              <button
                                ...
                                disabled={index === 0}
                                className="inline-flex items-center gap-1 rounded-full border border-white/10 bg-[rgba(18,18,26,0.92)] px-3 py-1 text-xs font-semibold text-slate-400 disabled:cursor-not-allowed disabled:opacity-40"
                              >
                                <ArrowUp size={12} />
                                Up
                              </button>
                            ...
                            {editingProduct.images.length > 1 ? (
                              <button
                                ...
                                className="inline-flex items-center gap-1 rounded-full border border-rose-500/20 bg-rose-500/10 px-3 py-1 text-xs font-semibold text-rose-300"
                              >
                                <X size={12} />
                                Remove
                              </button>
                      ...
                      <button
                        ...
                        className="inline-flex items-center gap-2 rounded-full border border-white/10 bg-[rgba(18,18,26,0.92)] px-3 py-2 text-xs font-semibold text-slate-300 hover:bg-white/5 disabled:cursor-not-allowed disabled:opacity-50"
                      >
                        <ImagePlus size={14} />
                        Add image
                      </button>
                ...
                {formError ? (
                  <div aria-live="polite" className="rounded-2xl border border-rose-500/20 bg-rose-500/10 px-4 py-3 text-sm font-medium text-rose-300">
                    {formError}
                  </div>
                ) : null}
          ...
          <section className="gc-card-compact space-y-4">
            <header className="border-b border-white/10 pb-4">
              ...
              <p className="mt-1 text-xs text-zinc-500">Monitor low-rated or frequently reviewed products without leaving the catalog screen.</p>
            </header>
            <div className="space-y-3">
              {reviewsQuery.isLoading ? <p aria-live="polite" className="text-sm text-zinc-500">Loading reviews…</p> : null}
              {!reviewsQuery.isLoading && reviews.length === 0 ? (
                <p className="text-sm text-zinc-500">No customer reviews yet.</p>
              ) : null}
              {reviews.map((review) => (
                <article key={review.productReviewId} className="rounded-2xl border border-white/10 bg-white/5 p-3">
                  <div className="flex items-start justify-between gap-3">
                    <div>
                      <p className="text-sm font-semibold text-white">{review.productName}</p>
                      <p className="text-[11px] text-zinc-500">by {review.customerName}</p>
                    </div>
                    <div className="inline-flex items-center gap-1 rounded-full bg-amber-500/10 px-2 py-1 text-[11px] font-semibold text-amber-300">
                      <Star size={12} className="fill-amber-400 text-amber-400" />
                      {review.rating}
                    </div>
                  </div>
                  {review.comment ? <p className="mt-2 text-xs text-slate-300">{review.comment}</p> : null}
                </article>
              ))}
            </div>
          </section>
