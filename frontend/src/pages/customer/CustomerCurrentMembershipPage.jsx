...
const statusTone = {
  ...
  SCHEDULED: 'bg-amber-100 text-amber-300',
  PENDING: 'bg-amber-100 text-amber-300',
...
function formatCouponBenefit(coupon, payment) {
  ...
  if (discountAmount > 0) {
    if (originalAmount > 0) {
      ...
      parts.push(`${formatInteger(discountAmount)} VND off (${discountPercent}% saved)`)
    } else {
      parts.push(`${formatInteger(discountAmount)} VND off`)
    }
  }
  ...
  return [
    {
      ...
      tone: validForCheckin ? 'text-emerald-600' : 'text-zinc-500',
    },
    {
      label: 'Coach booking',
      value: allowsCoachBooking ? 'Unlocked' : 'Not included',
      tone: allowsCoachBooking ? 'text-emerald-600' : 'text-zinc-500',
    },
    {
      label: 'Plan type',
  ...
  return (
    ...
    >
      {currentMembershipQuery.isLoading && (
        <div className="rounded-2xl border border-white/10 bg-[rgba(18,18,26,0.92)] p-5 text-sm text-zinc-500 shadow-sm">
          Loading membership details...
        </div>
      )}

      {currentMembershipQuery.isError && (
        <div className="rounded-2xl border border-rose-500/20 bg-rose-500/10 p-5 text-sm text-rose-300 shadow-sm">
          Failed to load your current membership.
        </div>
      )}

      {!currentMembershipQuery.isLoading && !currentMembershipQuery.isError && !hasMembership && (
        <section className="rounded-3xl border border-white/10 bg-[rgba(18,18,26,0.92)] p-6 shadow-sm">
          <div className="flex flex-col gap-5 md:flex-row md:items-center md:justify-between">
            <div className="space-y-2">
              <p className="text-sm font-semibold uppercase tracking-[0.18em] text-slate-400">No active membership</p>
              <h2 className="text-2xl font-bold text-white">You are not enrolled in a membership plan yet.</h2>
            ...
            <div className="flex flex-wrap gap-3">
              ...
              <Link
                ...
                className="inline-flex min-h-11 items-center justify-center rounded-full border border-white/10 bg-[rgba(18,18,26,0.92)] px-5 py-2.5 text-sm font-semibold text-slate-300 transition hover:bg-white/5"
              >
                View promotions
              </Link>
            </div>
      ...
      {!currentMembershipQuery.isLoading && !currentMembershipQuery.isError && hasMembership && (
        <div className="grid gap-6 xl:grid-cols-[1.25fr_0.95fr]">
          <section className="space-y-6">
            <article className="rounded-3xl border border-white/10 bg-[rgba(18,18,26,0.92)] p-6 shadow-sm">
              <div className="space-y-5">
                <div className="space-y-3">
                  ...
                  <div>
                    <h2 className="text-2xl font-bold text-white">{plan?.name || 'Membership plan'}</h2>
                ...
                <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
                  ...
                  <div className="rounded-2xl border border-gym-500/20 bg-gym-500/10 px-4 py-3">
                    <p className="text-[11px] font-semibold uppercase tracking-[0.18em] text-gym-300">Plan summary</p>
                    <div className="mt-3 space-y-3 text-sm text-slate-300">
                      <div className="flex items-center justify-between gap-3">
                        <span>Price paid</span>
                        <strong className="text-white">{formatCurrency(plan?.price)}</strong>
                      </div>
                      <div className="flex items-center justify-between gap-3">
                        <span>Duration</span>
                        <strong className="text-white">{plan?.durationDays || 0} day(s)</strong>
                      </div>
                      {'daysRemaining' in membership ? (
                        <div className="flex items-center justify-between gap-3">
                          <span>Time remaining</span>
                          <strong className="text-emerald-300">{membership.daysRemaining} day(s)</strong>
                        </div>
                      ) : null}
                      {'daysUntilActive' in membership ? (
                        <div className="flex items-center justify-between gap-3">
                          <span>Starts in</span>
                          <strong className="text-amber-300">{membership.daysUntilActive} day(s)</strong>
                        </div>
                      ) : null}
                    </div>
                  </div>
              ...
              <div className="mt-5 rounded-2xl border border-white/10 bg-white/5 px-4 py-3 text-sm">
                <p className={`font-semibold ${validForCheckin ? 'text-emerald-300' : 'text-amber-300'}`}>
            ...
            {queuedMembership && Object.keys(queuedMembership).length > 0 ? (
              <article className="rounded-3xl border border-amber-500/20 bg-amber-500/10 p-6 shadow-sm">
                <div className="flex flex-wrap items-center justify-between gap-3">
                  <div>
                    <p className="text-sm font-semibold uppercase tracking-[0.18em] text-amber-300">Queued next membership</p>
                  ...
                  <span className="rounded-full bg-amber-100 px-3 py-1 text-xs font-semibold text-amber-300">
                    {queuedMembership?.status || 'SCHEDULED'}
                  </span>
                </div>

                <div className="mt-4 grid gap-3 sm:grid-cols-3">
                  <div className="rounded-2xl border border-amber-500/20 bg-white/80 px-4 py-3">
            ...
            {payment ? (
              <article className="rounded-3xl border border-white/10 bg-[rgba(18,18,26,0.92)] p-6 shadow-sm">
                <div className="flex items-center gap-3">
                  <span className="rounded-2xl bg-white/10 p-3 text-slate-300">
                    <CreditCard size={18} />
                  </span>
                  <div>
                    <h3 className="text-lg font-bold text-white">Payment Snapshot</h3>
                    <p className="text-sm text-zinc-500">Latest recorded payment linked to this membership.</p>
                  </div>
                </div>
                ...
                <div className="mt-5 grid gap-3 sm:grid-cols-2">
                  <div className="rounded-2xl border border-white/10 bg-white/5 px-4 py-3">
                    ...
                    <p className="mt-2 text-sm font-semibold text-white">{formatCurrency(payment?.originalAmount ?? payment?.amount)}</p>
                  </div>
                  <div className="rounded-2xl border border-white/10 bg-white/5 px-4 py-3">
                    <p className="text-[11px] font-semibold uppercase tracking-[0.18em] text-slate-400">Payment status</p>
                    <p className="mt-2 text-sm font-semibold text-white">{payment?.status || '-'}</p>
                  </div>
                  <div className="rounded-2xl border border-white/10 bg-white/5 px-4 py-3">
                    <p className="text-[11px] font-semibold uppercase tracking-[0.18em] text-slate-400">Discount applied</p>
                    <p className="mt-2 text-sm font-semibold text-white">{formatCurrency(payment?.discountAmount)}</p>
                  </div>
                  <div className="rounded-2xl border border-white/10 bg-white/5 px-4 py-3">
                ...
                {coupon ? (
                  <div className="mt-5 rounded-2xl border border-gym-500/20 bg-gym-500/10 px-4 py-4">
                    <div className="flex flex-wrap items-center justify-between gap-3">
                      <div>
                        <p className="text-[11px] font-semibold uppercase tracking-[0.18em] text-gym-300">Applied coupon</p>
                        <p className="mt-2 text-lg font-bold text-white">{coupon?.promoCode || 'Membership coupon'}</p>
                      </div>
                      <span className="rounded-full bg-[rgba(18,18,26,0.92)] px-3 py-1 text-xs font-semibold text-gym-300 shadow-sm">
                        {coupon?.applyTarget || 'MEMBERSHIP'}
                      </span>
                    </div>
                    <p className="mt-3 text-sm font-semibold text-slate-100">{formatCouponBenefit(coupon, payment)}</p>
          ...
          <aside className="space-y-6">
            <article className="rounded-3xl border border-white/10 bg-[rgba(18,18,26,0.92)] p-6 shadow-sm">
              ...
              <p className="mt-1 text-sm text-zinc-500">Jump directly to the next task instead of browsing the full workspace.</p>

              <div className="mt-5 space-y-3">
                <Link
                  to="/customer/membership"
                  className="flex items-start gap-3 rounded-2xl border border-white/10 bg-white/5 px-4 py-3 transition hover:border-gym-500/20 hover:bg-gym-500/15"
                >
                  <span className="rounded-xl bg-[rgba(18,18,26,0.92)] p-2 text-gym-300 shadow-sm">
                    <CalendarClock size={16} />
                  </span>
                  <span>
                    <span className="block text-sm font-semibold text-white">Manage plans</span>
                    <span className="mt-1 block text-xs text-zinc-500">Renew, upgrade, or browse other membership options.</span>
                  </span>
                </Link>
                ...
                <Link
                  ...
                  className={`flex items-start gap-3 rounded-2xl border px-4 py-3 transition ${
                    allowsCoachBooking
                      ? 'border-white/10 bg-white/5 hover:border-gym-500/20 hover:bg-gym-500/15'
                ...
                >
                  ...
                  <span>
                    <span className="block text-sm font-semibold text-white">Coach booking</span>
                ...
                >
                  ...
                  <span>
                    <span className="block text-sm font-semibold text-white">Check-in and health</span>
                    <span className="mt-1 block text-xs text-zinc-500">Open your QR code and review body metrics in one place.</span>
                  </span>
                </Link>
              </div>
            </article>
            ...
            <article className="rounded-3xl border border-white/10 bg-[rgba(18,18,26,0.92)] p-6 shadow-sm">
              <div className="flex items-center gap-3">
                <span className="rounded-2xl bg-emerald-500/15 p-3 text-emerald-300">
                  <BadgeCheck size={18} />
                </span>
                <div>
                  <h3 className="text-lg font-bold text-white">Membership Guidance</h3>
                  <p className="text-sm text-zinc-500">What to expect with your current plan.</p>
                </div>
              </div>
