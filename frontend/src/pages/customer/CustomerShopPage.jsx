        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-gym-dark-900/60 backdrop-blur-md">
          <div className="animate-in fade-in zoom-in rounded-[40px] bg-white p-12 text-center shadow-2xl duration-300 max-w-sm border border-gym-dark-100">
            <div className="mx-auto mb-6 flex h-20 w-20 items-center justify-center rounded-2xl bg-gym-50 text-gym-500 shadow-inner">
              <Star size={40} className="fill-gym-500" />
            </div>
            <h2 className="mb-3 text-3xl font-black text-gym-dark-900 tracking-tight">Order Placed!</h2>
            <p className="text-gym-dark-400 font-bold leading-relaxed">Your gear is ready for pickup at the front desk. Fuel your gains!</p>
          </div>
        </div>
      )}

      {showCheckoutModal && (
        <div className="fixed inset-0 z-[90] flex items-center justify-center bg-gym-dark-900/60 backdrop-blur-md px-4">
          <div className="w-full max-w-md animate-in fade-in zoom-in rounded-[32px] bg-white p-8 shadow-2xl duration-200 border border-gym-dark-100">
            <h3 className="mb-6 text-2xl font-black text-gym-dark-900 tracking-tight">Finalize Order</h3>
            <div className="space-y-5">
              <p className="rounded-xl border-2 border-gym-100 bg-gym-50 px-4 py-3 text-xs font-bold text-gym-800 leading-relaxed">
                🚀 Note: All products are for in-gym pickup. Show your order ID at the front desk.
              </p>
              <div>
                <label className="mb-2 block text-xs font-black uppercase tracking-widest text-gym-dark-400">Payment Gateway</label>
                <select
                  className="w-full rounded-xl border-2 border-gym-dark-50 bg-white px-4 py-3 text-sm font-bold focus:border-gym-500 focus:outline-none transition-all"
                  value={checkoutOptions.paymentMethod}
                  onChange={(e) => setCheckoutOptions({ ...checkoutOptions, paymentMethod: e.target.value })}
                >
                  <option value="PAYOS">PayOS (Vietnam Bank Transfer/QR)</option>
                </select>
              </div>
              <div>
                <label className="mb-2 block text-xs font-black uppercase tracking-widest text-gym-dark-400">Promo Code</label>
                <select
                  className="w-full rounded-xl border-2 border-gym-dark-50 bg-white px-4 py-3 text-sm font-bold focus:border-gym-500 focus:outline-none transition-all"
                ...
                {productCoupons.length === 0 && (
                  <p className="mt-2 text-[10px] font-bold text-gym-dark-300 italic">No available coupons. Visit the Promotions page!</p>
                )}
                {couponPreview && checkoutOptions.promoCode && (
                  <div className="mt-3 rounded-xl border-2 border-gym-200 bg-gym-50/50 p-3 text-[11px] font-black uppercase tracking-tight text-gym-700 flex justify-between">
                    <span>Discount Applied</span>
                    <span>-{Number(couponPreview.estimatedDiscount || 0).toLocaleString()} VND</span>
                  </div>
                )}
                {couponPreviewError && (
            ...
            <div className="mt-8 flex gap-3">
              <button
                ...
                className="flex-1 rounded-xl border-2 border-gym-dark-100 py-3 text-sm font-black text-gym-dark-400 hover:bg-gym-dark-50 transition-all"
              >
                Cancel
              </button>
              <button
                onClick={confirmCheckout}
                className="flex-1 btn-primary py-3 text-sm shadow-xl"
              >
                Confirm & Pay
              </button>
            </div>
          ...
          <div className="space-y-3 max-h-[520px] overflow-y-auto pr-2 custom-scrollbar">
            ...
            {filteredProducts.map((product) => (
              <button
                ...
                className={`flex w-full items-center justify-between rounded-2xl border-2 px-4 py-4 text-left transition-all duration-300 ${selectedProductId === product.productId
                  ? 'border-gym-500 bg-gym-50 shadow-md scale-[1.01]'
                  : 'border-gym-dark-50 bg-white hover:border-gym-200'
                  }`}
              >
                <div className="space-y-1.5 flex-1 min-w-0 pr-4">
                  <p className="text-sm font-black text-gym-dark-900 truncate">{product.name}</p>
                  <p className="line-clamp-1 text-[11px] font-bold text-gym-dark-400">{product.description}</p>
                  <ProductRating rating={product.averageRating} count={product.reviewCount} />
                </div>
                <div className="flex flex-col items-end gap-3 shrink-0">
                  <p className="text-sm font-black text-gym-500">
                    {product.price?.toLocaleString('en-US')} <span className="text-[10px] text-gym-dark-300 uppercase">VND</span>
                  </p>
                  <div className="inline-flex items-center gap-1 rounded-xl border-2 border-gym-dark-50 bg-white px-2 py-1 text-xs font-black">
                    <button
                      type="button"
                      className="w-6 h-6 flex items-center justify-center rounded-lg text-gym-dark-300 hover:bg-gym-dark-50 hover:text-gym-dark-900 transition-colors"
                      onClick={(event) => {
                    ...
                    <span className="min-w-[1.25rem] text-center text-gym-dark-900">
                  ...
                  <div className="flex items-center gap-2">
                    <button
                      ...
                      className="inline-flex items-center gap-2 rounded-xl bg-gym-dark-900 px-4 py-2 text-[10px] font-black text-white hover:bg-gym-dark-800 transition-colors"
                    >
                    ...
                    <button
                      ...
                      className="inline-flex items-center gap-2 rounded-xl bg-gym-500 px-4 py-2 text-[10px] font-black text-white hover:bg-gym-600 transition-colors shadow-lg shadow-gym-500/20"
                    >
                      <Zap size={14} />
                    </button>
                  </div>
        ...
        <section className="space-y-4 gc-card-compact">
          ...
          {selectedProductId && productDetail && (
            <div className="space-y-4">
              <article className="grid gap-6 rounded-2xl border border-gym-dark-50 bg-gym-dark-50/20 p-6 md:grid-cols-[minmax(0,2fr)_minmax(0,1.5fr)]">
                <div className="space-y-3">
                  <h3 className="text-xl font-black text-gym-dark-900 tracking-tight">{productDetail.product.name}</h3>
                  <ProductRating
                    rating={productDetail.product.averageRating}
                    count={productDetail.product.reviewCount}
                  />
                  <p className="text-sm font-medium text-gym-dark-400 leading-relaxed">{productDetail.product.description}</p>
                </div>
                <div className="flex flex-col items-end justify-between gap-4">
                  <p className="text-2xl font-black text-gym-500">
                    {productDetail.product.price?.toLocaleString('en-US')}{' '}
                    <span className="text-[10px] font-black text-gym-dark-300 uppercase">VND</span>
                  </p>
                  <div className="inline-flex items-center gap-2 rounded-xl border-2 border-gym-dark-50 bg-white px-3 py-1.5 text-sm font-black">
                    <button
                      type="button"
                      className="w-8 h-8 flex items-center justify-center rounded-lg text-gym-dark-300 hover:bg-gym-dark-50 hover:text-gym-dark-900 transition-colors"
                      onClick={() => changeDesiredQuantity(productDetail.product.productId, -1)}
                    >
                      -
                    </button>
                    <span className="min-w-[2.5rem] text-center text-gym-dark-900">
                  ...
                  <div className="flex flex-wrap justify-end gap-3">
                    <button
                      ...
                      className="inline-flex items-center gap-3 rounded-2xl bg-gym-dark-900 px-6 py-3 text-sm font-black text-white hover:bg-gym-dark-800 transition-all shadow-lg"
                    >
                    ...
                    <button
                      ...
                      className="btn-primary px-6 py-3 text-sm shadow-xl"
                    >
                      <Zap size={18} />
                      Buy Now
                    </button>
              ...
              <article className="space-y-4 rounded-2xl border border-gym-dark-50 bg-white p-6 shadow-sm">
                <h4 className="text-xs font-black uppercase tracking-widest text-gym-dark-400">Customer feedback</h4>
                {productDetail.reviews.length === 0 && (
                  <p className="text-xs font-bold text-gym-dark-300 italic">No reviews yet. Be the first!</p>
                )}
                <div className="space-y-3 max-h-64 overflow-y-auto pr-2 custom-scrollbar">
                  {productDetail.reviews.map((review) => (
                    <div
                      key={review.productReviewId}
                      className="space-y-2 rounded-xl border border-gym-dark-50 bg-gym-dark-50/30 p-4 transition-hover hover:bg-gym-dark-50/50"
                    >
                      <div className="flex items-center justify-between gap-3">
                        <p className="text-xs font-black text-gym-dark-900 uppercase tracking-tight">{review.customerName}</p>
                        <ProductRating rating={review.rating} size="sm" />
                      </div>
                      {review.comment && <p className="text-sm font-medium text-gym-dark-700 leading-relaxed">{review.comment}</p>}
                      <p className="text-[10px] font-bold text-gym-dark-300">
      ...
      >
        <div className="flex h-full flex-col">
          ...
          <div className="flex-1 space-y-4 overflow-y-auto px-6 py-6 custom-scrollbar">
            <div className="space-y-4">
              {cartItems.length === 0 && (
                <div className="flex flex-col items-center justify-center rounded-[32px] border-2 border-dashed border-gym-dark-100 bg-gym-dark-50/30 py-12 px-6 text-center">
                  <div className="mb-4 rounded-full bg-white p-4 text-gym-dark-200">
                    <ShoppingCart size={40} />
                  </div>
                  <p className="text-sm font-black text-gym-dark-400 uppercase tracking-widest">Cart is empty</p>
                  <p className="mt-2 text-xs font-bold text-gym-dark-300">Fuel your workout, add some gear!</p>
                </div>
              )}
              {cartItems.map((item) => (
                <article
                  key={item.productId}
                  className="space-y-3 rounded-[24px] border-2 border-gym-dark-50 bg-white p-4 shadow-sm transition-hover hover:border-gym-200"
                >
                  <div className="flex items-start justify-between gap-4">
                    <div className="min-w-0">
                      <p className="text-sm font-black text-gym-dark-900 truncate uppercase tracking-tight">{item.name}</p>
                      <p className="text-[11px] font-bold text-gym-dark-300">{Number(item.price || 0).toLocaleString('en-US')} VND <span className="text-[9px] opacity-60">/ UNIT</span></p>
                    </div>
                    <button
                      type="button"
                  ...
                  <div className="flex items-center justify-between gap-4 pt-2 border-t border-gym-dark-50">
                    <div className="inline-flex items-center gap-1 rounded-xl border-2 border-gym-dark-50 bg-gym-dark-50/30 px-2 py-1 text-sm font-black">
                      <button
                        type="button"
                        onClick={() => onUpdateQuantity(item.productId, Number(item.quantity || 0) - 1)}
                        className="w-6 h-6 flex items-center justify-center rounded-lg text-gym-dark-300 hover:text-gym-dark-900 transition-colors"
                      >
                        -
                      </button>
                      <span className="min-w-[1.5rem] text-center text-gym-dark-900">{item.quantity}</span>
                      <button
                        type="button"
                        onClick={() => onUpdateQuantity(item.productId, Number(item.quantity || 0) + 1)}
                        className="w-6 h-6 flex items-center justify-center rounded-lg text-gym-dark-300 hover:bg-gym-dark-50 hover:text-gym-dark-900 transition-colors"
                    ...
                    <p className="text-sm font-black text-gym-500">
                      {Number(item.lineTotal || 0).toLocaleString('en-US')} <span className="text-[9px]">VND</span>
                    </p>
                  </div>
                </article>
              ))}
            ...
            <section className="mt-8 space-y-4 rounded-[32px] border-2 border-gym-dark-900 bg-gym-dark-900 p-8 text-white shadow-2xl">
              <div className="flex items-center justify-between text-xs font-black uppercase tracking-widest text-gym-dark-400">
                <span>Estimated Total</span>
                <span className="text-xl text-gym-500 font-black not-italic">{Number(cart.subtotal || 0).toLocaleString('en-US')} VND</span>
              </div>
              ...
              <button
                ...
                onClick={onCheckout}
                className="btn-primary w-full py-4 text-base font-black shadow-xl"
              >
                {checkoutMutation.isPending ? 'Processing...' : 'Complete Purchase'}
              </button>

              <p className="text-[9px] text-center font-bold text-gym-dark-500 uppercase tracking-widest">
                Pickup required at front desk
              </p>
            </section>
          </div>
          ...
          <CartHistorySection orders={orders} />
        </div>
      </aside>
    </>
  )
...
function CartHistorySection({ orders }) {
  return (
    <section className="mt-4 space-y-4 rounded-[32px] border border-gym-dark-50 bg-gym-dark-50/20 p-6">
      <header className="flex items-center gap-3">
        <div className="rounded-xl bg-gym-dark-900 p-2 text-gym-500 shadow-lg">
          <History size={16} />
        </div>
        <div>
          <h3 className="text-xs font-black uppercase tracking-widest text-gym-dark-900">Order History</h3>
          <p className="text-[10px] font-bold text-gym-dark-400">Your recent gear hauls</p>
        </div>
      </header>
      <div className="space-y-3 max-h-72 overflow-y-auto pr-2 custom-scrollbar">
        {orders.length === 0 && (
          <p className="text-xs font-bold text-gym-dark-300 italic py-4">No orders yet. Start your journey!</p>
        )}
        {orders.map((order) => (
          <div
            key={order.orderId}
            className="group space-y-3 rounded-2xl border-2 border-gym-dark-50 bg-white p-4 transition-hover hover:border-gym-200 shadow-sm"
          >
            <div className="flex items-center justify-between gap-3">
              <p className="text-xs font-black text-gym-dark-900 uppercase">
                #{order.orderId} —{' '}
                <span
                  className={
                    order.status === 'PAID'
            ...
            <ul className="space-y-1">
              {order.items?.map((item) => (
                <li key={item.productId} className="text-[11px] font-bold text-gym-dark-500 flex justify-between">
                  <span>{item.name}</span>
                  <span className="text-gym-dark-300">x{item.quantity}</span>
                </li>
              ))}
            </ul>
            <p className="text-sm font-black text-gym-dark-900 pt-2 border-t border-gym-dark-50 text-right">
              {Number(order.totalAmount || 0).toLocaleString('en-US')} <span className="text-[9px] text-gym-dark-300 uppercase">VND</span>
            </p>
          </div>
        ))}
      </div>
    ...
    <div className="flex items-center gap-2">
      <div className="flex items-center gap-[2px]">
        {Array.from({ length: totalStars }).map((_, index) => {
          ...
          return <StarOff key={index} size={iconSize} className="text-gym-dark-100" />
        })}
      </div>
      <span className="text-[11px] font-black text-gym-dark-300 uppercase tracking-tighter">
