...
function CustomerKnowledgePage() {
  ...
  const initialPendingKnowledgeTarget = useMemo(() => {
    if (typeof window === 'undefined') {
      return { tab: 'WORKOUTS', workoutId: null, foodId: null, scrollWorkoutVideo: false, scrollFoodDetail: false }
    }
    const workoutId = Number(window.sessionStorage.getItem('gymcore.knowledge.openWorkoutId') || '')
    const foodId = Number(window.sessionStorage.getItem('gymcore.knowledge.openFoodId') || '')
    const scrollWorkoutVideo = window.sessionStorage.getItem('gymcore.knowledge.scrollWorkoutVideo') === '1'
    const scrollFoodDetail = window.sessionStorage.getItem('gymcore.knowledge.scrollFoodDetail') === '1'
    window.sessionStorage.removeItem('gymcore.knowledge.openWorkoutId')
    window.sessionStorage.removeItem('gymcore.knowledge.openFoodId')
    window.sessionStorage.removeItem('gymcore.knowledge.scrollWorkoutVideo')
    window.sessionStorage.removeItem('gymcore.knowledge.scrollFoodDetail')
    if (Number.isFinite(workoutId) && workoutId > 0) {
      return { tab: 'WORKOUTS', workoutId, foodId: null, scrollWorkoutVideo, scrollFoodDetail: false }
    }
    if (Number.isFinite(foodId) && foodId > 0) {
      return { tab: 'FOODS', workoutId: null, foodId, scrollWorkoutVideo: false, scrollFoodDetail }
    }
    return { tab: 'WORKOUTS', workoutId: null, foodId: null, scrollWorkoutVideo: false, scrollFoodDetail: false }
  }, [])
  const [activeTab, setActiveTab] = useState(initialPendingKnowledgeTarget.tab)

  const [workoutCategoryId, setWorkoutCategoryId] = useState('ALL')
  const [workoutSearch, setWorkoutSearch] = useState('')
  const [selectedWorkoutId, setSelectedWorkoutId] = useState(initialPendingKnowledgeTarget.workoutId)

  const [foodCategoryId, setFoodCategoryId] = useState('ALL')
  const [foodSearch, setFoodSearch] = useState('')
  const [selectedFoodId, setSelectedFoodId] = useState(initialPendingKnowledgeTarget.foodId)
  ...
  const scrollToWorkoutVideoAfterLoadRef = useRef(Boolean(initialPendingKnowledgeTarget.scrollWorkoutVideo))
  const scrollToFoodDetailAfterLoadRef = useRef(Boolean(initialPendingKnowledgeTarget.scrollFoodDetail))
  ...
  useEffect(() => {
    if (!selectedWorkoutId || !workoutDetailQuery.data) return
    if (workoutDetailQuery.data.workoutId !== selectedWorkoutId) return
    if (!scrollToWorkoutVideoAfterLoadRef.current) return
    scrollToWorkoutVideoAfterLoadRef.current = false
    const videoUrl = workoutDetailQuery.data.videoUrl
    if (!toYouTubeEmbedUrl(videoUrl)) return
    const run = () => document.getElementById('gymcore-knowledge-workout-video')?.scrollIntoView({ behavior: 'smooth', block: 'center' })
    requestAnimationFrame(() => requestAnimationFrame(run))
  }, [selectedWorkoutId, workoutDetailQuery.data])

  useEffect(() => {
    if (!selectedFoodId || !foodDetailQuery.data) return
    if (foodDetailQuery.data.foodId !== selectedFoodId) return
    if (!scrollToFoodDetailAfterLoadRef.current) return
    scrollToFoodDetailAfterLoadRef.current = false
    const run = () => document.getElementById('gymcore-knowledge-food-detail')?.scrollIntoView({ behavior: 'smooth', block: 'center' })
    requestAnimationFrame(() => requestAnimationFrame(run))
  }, [selectedFoodId, foodDetailQuery.data])

  useEffect(() => {
    const openWorkout = (event) => {
      const workoutId = Number(event?.detail?.workoutId)
      if (!Number.isFinite(workoutId) || workoutId <= 0) return
      scrollToWorkoutVideoAfterLoadRef.current = event.detail?.scrollToVideo === true
      setActiveTab('WORKOUTS')
      setSelectedWorkoutId(workoutId)
    }
    const openFood = (event) => {
      const foodId = Number(event?.detail?.foodId)
      if (!Number.isFinite(foodId) || foodId <= 0) return
      scrollToFoodDetailAfterLoadRef.current = event.detail?.scrollToDetail === true
      setActiveTab('FOODS')
      setSelectedFoodId(foodId)
    }
    window.addEventListener('gymcore:knowledge-open-workout', openWorkout)
    window.addEventListener('gymcore:knowledge-open-food', openFood)
    return () => {
      window.removeEventListener('gymcore:knowledge-open-workout', openWorkout)
      window.removeEventListener('gymcore:knowledge-open-food', openFood)
    }
  }, [])
    ...
    >
      ...
      <header className="overflow-hidden rounded-[32px] border border-white/10 bg-[radial-gradient(circle_at_top_right,_rgba(245,158,11,0.18),_transparent_34%),linear-gradient(135deg,_rgba(18,18,26,0.98),_rgba(10,10,15,0.92))] p-6 shadow-[0_24px_60px_rgba(0,0,0,0.32)] backdrop-blur-xl">
          <div className="flex flex-col gap-6 xl:flex-row xl:items-start xl:justify-between">
            <div className="max-w-3xl">
              ...
              <p className="mt-3 max-w-2xl text-sm leading-7 text-slate-600">
                Browse workouts and foods, save your fitness goals, and ask the workout assistant for quick, practical guidance.
              </p>
            </div>
        ...
        <div className="rounded-[32px] border border-slate-200 bg-white p-5 shadow-sm">
          <div className="rounded-3xl border border-amber-300/60 bg-[linear-gradient(135deg,rgba(28,25,23,0.96),rgba(17,24,39,0.94)_55%,rgba(120,53,15,0.92))] p-4 shadow-[0_16px_40px_rgba(15,23,42,0.35)]">
              <div className="flex flex-wrap items-start justify-between gap-3">
                <div>
                  <p className="text-xs font-semibold uppercase tracking-[0.2em] text-amber-200">Saved fitness goals</p>
                  <p className="mt-1 text-sm text-slate-100">Save your goals once so recommendations across Workout and Food stay consistent.</p>
                </div>
                <div className="flex flex-wrap gap-2">
              <button
                type="button"
                ...
                className="rounded-full bg-amber-500 px-4 py-2 text-sm font-semibold text-slate-950 transition hover:bg-amber-400 disabled:cursor-not-allowed disabled:opacity-60"
              >
              ...
              {customerGoalsQuery.isLoading ? <p className="mt-3 text-sm text-slate-200">Loading saved goals...</p> : null}
                ...
                {workoutDetailQuery.data ? (
                  <div className="space-y-3">
                    ...
                    {toYouTubeEmbedUrl(workoutDetailQuery.data.videoUrl) ? (
                      <div
                        id="gymcore-knowledge-workout-video"
                        className="overflow-hidden rounded-2xl border border-slate-200 bg-slate-50 scroll-mt-24"
                      >
        ...
        {activeTab === 'FOODS' ? (
          <div className="space-y-4">
            ...
            {selectedFoodId ? (
              <div
                id="gymcore-knowledge-food-detail"
                className="rounded-3xl border border-slate-200 bg-white p-5 scroll-mt-24"
              >
